...
export class Ui {
   ...
   *
   * Во время партии она нужна включённой, хотя её жест (свайп от края экрана
   * на Android) и прилетает посреди игры. Пока она видима, жест приходит к нам
   * и мы ставим паузу — партию можно продолжить. Если её спрятать, тот же жест
   * свернёт Mini App целиком, и забег просто потеряется. Случайная пауза
   * неприятна, потерянная партия — хуже.
   */
  _syncBackButton() {
