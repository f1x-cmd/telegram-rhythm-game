...
export const OFFICE = {
  ...
  hitPad: 1.22,
  bladeLife: 0.42,
  // Разрез по стрелке. Допуск ±60°: направления стоят через 90°, так что
  // соседние конусы чуть перекрываются (диагональ засчитается обеим стрелкам),
  // а перпендикуляр и встречное движение — уже нет. Прежние ±120° давали конус
  // в 240° из 360°, то есть две трети случайных свайпов считались верными.
  dirToleranceDeg: 60,
  // Короче этого жест не задаёт направления: тап по стрелке не считается
  dirMinPx: 26,
  // Серая бомба ослепляет почти полностью и держит это, чтобы реально мешать:
  // сначала dustHold секунд на полной непрозрачности, потом рассеивание.
  // Итого ~2 с ощутимой помехи — а не вспышка, которую не успеваешь заметить.
  dustAmount: 1,
  dustHold: 0.8,
  dustDecay: 0.55,
};
...
export const DRIVE_DIFFICULTY = {
  easy: {
    ...
    fallGravity: 0.24, fallVy: { min: 0.04, max: 0.075 }, sizeScale: 1.18,
  },
  medium: {
    ...
    fallGravity: 0.44, fallVy: { min: 0.1, max: 0.16 }, sizeScale: 1.22,
  },
  hard: {
    ...
    fallGravity: 0.54, fallVy: { min: 0.12, max: 0.19 }, sizeScale: 1.28,
  },
};
 ...
 * Пароля нет: ID берётся из initDataUnsafe, поэтому панель открывается
 * лишь при запуске как Mini App — в обычной вкладке браузера вход не выйдет.
 * Список публичный по своей природе, и это не дыра: бэкенда нет, панель
 * правит настройки только на том устройстве, где открыта.
 */
export const ADMIN = {
  ids: ['348526969'], // @yacenter
};
